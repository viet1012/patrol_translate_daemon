...
class PatrolTranslateUi:
    def __init__(self, root: tk.Tk, cfg: Config):
        ...
        self.load_generation = 0
        self.auto_after_id: Optional[str] = None
        self.auto_load_generation: Optional[int] = None
        self.auto_pending_after_load = 0
        self.last_records_signature: tuple[Any, ...] = ()
    ...
    def _rows_signature(rows: list[dict[str, Any]]) -> tuple[Any, ...]:
        """Tạo dấu vân tay để biết dữ liệu hiển thị có thực sự thay đổi."""
        return tuple(
            (
                str(row.get("record_id", "")),
                str(row.get("source_column", "")),
                str(row.get("target_column", "")),
                normalize(row.get("source")) or "",
                normalize(row.get("target")) or "",
                str(row.get("qr_key", "")),
    ...
    def _replace_tree_rows(self, rows: list[dict[str, Any]]) -> int:
        """Cập nhật bảng nhưng giữ selection và vị trí cuộn."""
        ...
        for index, row in enumerate(rows, start=1):
            ...
            self.records[item_id] = row
            self._render_record(item_id)
            if not normalize(row.get("target")):
                pending += 1

        restored = [iid for iid in selected_ids if self.tree.exists(iid)]
        if restored:
    ...
    def load_records(
        ...
        *,
        ...
        generation = self.load_generation
        if auto_translate:
            self.auto_load_generation = generation
            self.auto_pending_after_load = 0

        self.status_var.set(
            "Đang kiểm tra dữ liệu mới..." if auto_translate
    ...
    def _auto_tick(self) -> None:
        ...
        # Auto chỉ quét DB trước. Chỉ khi tìm thấy dòng chưa dịch
        # mới khởi động worker AI. Nhờ vậy không gọi AI vô ích và
        # không reload/nhấp nháy bảng khi dữ liệu không đổi.
        if not self.worker_running and not self.loading_running:
            self.load_records(show_error=False, auto_translate=True)
        ...
        try:
            while True:
                ...
                if event_name == "records":
                    ...
                    is_auto_load = generation == self.auto_load_generation

                    if signature != self.last_records_signature:
                        pending = self._replace_tree_rows(rows)
                        self.last_records_signature = signature
                        self.status_var.set(
                            "Phát hiện dữ liệu mới." if is_auto_load
                            else "Đã tải danh sách."
                        )
                    elif is_auto_load:
                        self.status_var.set(
                            f"Auto: không có thay đổi, còn {pending} dòng chờ dịch."
                            if pending
                            else "Auto: không có dữ liệu mới cần dịch."
                        )
                    else:
                        self.status_var.set("Dữ liệu không thay đổi.")
                    ...
                    if is_auto_load:
                        self.auto_pending_after_load = pending
                    ...
                    if generation == self.load_generation:
                        ...
                        should_auto_translate = (
                            self.auto_var.get()
                            and generation == self.auto_load_generation
                            and self.auto_pending_after_load > 0
                            and not self.worker_running
                        )

                        if generation == self.auto_load_generation:
                            self.auto_load_generation = None

                        if should_auto_translate:
                            self.status_var.set(
                                f"Auto: bắt đầu dịch "
                                f"{self.auto_pending_after_load} dòng/cặp..."
                            )
                            self.auto_pending_after_load = 0
                            self.translate_now()
                    ...
                    if generation == self.load_generation:
                        if generation == self.auto_load_generation:
                            self.auto_load_generation = None
                            self.auto_pending_after_load = 0
                            # Auto chạy nền nên không bật popup làm gián đoạn người dùng.
                            self.status_var.set(f"Auto kiểm tra lỗi: {message}")
                        else:
                            messagebox.showerror("Lỗi", str(message))
                            self.status_var.set("Không tải được dữ liệu.")
